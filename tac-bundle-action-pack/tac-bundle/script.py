...
if (int(vl[1]) > 26) or (int(vl[1]) == 26 and int(regex_new_version) >= 1):
    if not srn:
...
# Otherwise collect each logs individually
    ...
    baseline = [
        (
            "bash timeout 600 sudo tar -cvf - /mnt/flash/schedule/tech-support/* "
    ...
    baseline.append(
        (
            "bash timeout 600 sudo tar -cvf - /mnt/flash/TAC-* "
            f"> /mnt/flash/support-bundle{tar_prefix}-{dt}.tar"
        )
    )
ctx.info(f"Gathering baseline logs from device {ctx.getDevice().ip}")
ctx.runDeviceCmds(baseline, fmt="text", timeout=800, diConnTimeout=600, diCliTimeout=600)
